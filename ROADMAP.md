...
| Статус | Задача | Описание | Сложность |
|--------|--------|----------|-----------|
| ✅ | Mark all as Read не работает | После нажатия "Отметить все прочитанными" и последующего обновления (refresh) UI не отражает прочитанное состояние — бейджи остаются, новости визуально не помечены. **Причина (2 бага)**: (1) `useMarkAllRead.onSuccess` не очищал `pendingCounts[channelId]` в `uiStore` — бейдж = `unreadCount + pendingCounts`. (2) Fetch-роут удаляет все `isRead=1` новости перед скачкой новых (`DELETE WHERE is_read=1`), а `/read-all` не обновлял `lastReadAt` в таблице каналов → после удаления сервер переfetch'ил их заново из Telegram с `isRead=0`. **Фикс**: (1) добавить `clearPendingCount(channelId)` в `onSuccess`; (2) в `/read-all` обновлять `lastReadAt = max(news.postedAt)` для канала. | ⭐⭐ |
| ⬜ | Превью не обновляется при частичной 429 | Если Telegram вернул 429 в процессе скачки, но основное изображение успело загрузиться, а превью (thumbnail) — нет: `localMediaPath` обновляется, но `thumbnailPath` остаётся пустым. Нужно дополнительный UPDATE для thumbnail после успешной скачки основного файла. | ⭐⭐ |
| ⬜ | Выгорание ссылок на download content | Ссылки на медиа-файлы (Telegram file references) имеют TTL и протухают. При попытке скачать устаревшую ссылку задача падает с ошибкой. Нужен механизм: повторный fetch `fetchMessageById` для получения свежей ссылки перед ретраем. | ⭐⭐⭐ |
| ⬜ | Слетает Telegram сессия (AUTH_KEY_UNREGISTERED) | `AUTH_KEY_UNREGISTERED` при обращении к Telegram — сессия протухла. Нужно: (A) уведомление через alertBot + в интерфейсе (баннер/badge в AppHeader); (B) исследовать возможность автоматического рефреша сессии через `tg.connect()` без полного `tg:auth`. | ⭐⭐⭐⭐ |
...
| ⬜ | Сортировка фильтров по количеству срабатываний | В `FilterPanel` таблица фильтров должна по умолчанию сортироваться по убыванию `hit_count` (сколько раз фильтр сработал на текущей выборке новостей). Добавить колонку `Hits` с числом, сортируемую по клику. | ⭐ |
| ⬜ | Иконка приложения (favicon / PWA) | Придумать и нарисовать иконку: favicon.ico, 192×192 и 512×512 PNG для PWA manifest. Можно использовать стилизованный символ 📰 или TG + газета. | ⭐ |
| ⬜ | Ссылки на каналы | В `ChannelItem` / шапке `NewsFeed` добавить кнопку-ссылку `https://t.me/{telegramId}`, открывающую канал в Telegram в новой вкладке. | ⭐ |
| ⬜ | Boss key для групп | Горячая клавиша (например, `B` или `Esc Esc`) мгновенно скрывает весь контент защищённых PIN-групп — сворачивает их в боковой панели и очищает `NewsDetail`. Актуально для NSFW-групп. | ⭐⭐ |
...
| ⬜ | Стилизация блоков просмотра новости | Каждый смысловой блок в `NewsDetailBody` помещается в отдельный визуальный контейнер: (1) медиа (фото/видео/альбом) — без бордера, фон прозрачный, скруглённые углы; (2) текст поста — карточка с видимым фоном (`colorBgContainer`) и бордером (`colorBorderSecondary`); (3) Instant View — стилизовать как в Telegram: серый фон секций, горизонтальные разделители, цитаты с левым бордером, заголовки H1–H3 с типографикой. Переходы между блоками — `gap` без сплошной полосы. | ⭐⭐⭐ |
| ⬜ | Шаринг новости | Кнопка «Поделиться» в `NewsDetailToolbar`. **Мобильные**: если `navigator.share` доступен (Web Share API) — нативный шаринг между приложениями (текст заголовка + ссылка на оригинальный пост в Telegram). **Десктоп / fallback**: копирует в буфер обмена краткое резюме новости: заголовок (первые ~100 символов текста) + ссылка `https://t.me/{channelTelegramId}/{msgId}`; после копирования — toast «Скопировано». Детектирование через `navigator.canShare?.()` или простую проверку `'share' in navigator`. | ⭐⭐ |
...
- [x] Проверка дубликата при добавлении канала: нормализуем telegramId на клиенте (strip @, t.me/, path) → сравниваем с `allChannels` на blur → ошибка прямо в поле; 409 от сервера тоже обрабатывается как fallback
